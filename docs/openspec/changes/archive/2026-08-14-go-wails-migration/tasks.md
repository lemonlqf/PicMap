...
- [x] **T5.4** 移除 axios 依赖和相关配置（`package.json`, `http/` 目录）
- [x] **T5.5** 调整 package.json 构建脚本适配 Wails
- [x] **T5.6** 复制前端到 `frontend/` 并验证 `npm run build` 正常

## 阶段六：集成测试与打包

- [x] **T6.1** 使用 `wails dev` 本地调试，验证所有 API 功能
- [x] **T6.2** 测试数据兼容性（用现有 `D:\PicMap\` 数据目录启动）
- [x] **T6.3** 性能测试：50 张图片并发上传对比 Node.js 版
- [x] **T6.4** 使用 `wails build -platform windows/amd64` 打包
- [x] **T6.5** 验证安装包大小（目标 < 25MB）和启动速度
- [x] **T6.6** 回归测试所有功能（地图、分组、时间轴、GPX、备份、多用户）
...
- [x] **P3.4** 手工验证：单张 50MB HEIC 上传前端内存不随图暴涨、无卡顿
- [x] **P3.5** 手工验证：批量多张含 GPS 图片上传成功、地图标记正确、进度可追踪
- [x] **P3.6** 手工验证：文件在确认前被删除时返回明确错误、不写脏数据、不影响同批其他图片
