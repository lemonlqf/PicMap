...
PicMap 是一个图片标注与管理平台的后端 API 服务，提供用户管理、图片上传下载、轨迹管理、数据备份等功能。
...
将 HEIC/HEIF/RAW 等格式转换为 JPEG。

**请求地址**: `POST /image/getJPGImage`
...
```json
{
  "code": 200,
  "msg": "成功",
  "data": "{\"verison\":\"1.0.0\",\"mapInfo\":{...},\"groupInfo\":[],\"imageInfo\":[],\"trackInfo\":[]}",
  "time": 1704067200000
}
```

> 注意: 返回的 data 字段为 JSON 字符串，需要 JSON.parse() 解析
...
|---------------|--------|------|----------|
| currentUserId | string | 是   | 当前用户ID |
| schema        | string | 是   | Schema 数据 (JSON字符串) |
...
{
  "currentUserId": "user123",
  "schema": "{\"verison\":\"1.0.0\",\"mapInfo\":{...},\"groupInfo\":[],\"imageInfo\":[]}"
}
```
...
**请求地址**: `POST /track/uploadTrack`

**请求参数**: FormData

| 参数名        | 类型   | 必填 | 说明     |
|---------------|--------|------|----------|
| file          | file   | 是   | GPX 轨迹文件 |
| currentUserId | string | 是   | 当前用户ID |
...
**请求地址**: `DELETE /track/deleteTrack`

**Query 参数**:

| 参数名        | 类型   | 必填 | 说明     |
|---------------|--------|------|----------|
| fileName      | string | 是   | 轨迹文件名 (原始名，不带前缀) |
| currentUserId | string | 是   | 当前用户ID |

**请求示例**:

```
DELETE /track/deleteTrack?fileName=跑步.gpx&currentUserId=user123
```
...
```
GET /track/getTrack?fileName=跑步.gpx&currentUserId=user123
```
