/*
 * @Author: Do not edit
 * @Date: 2025-07-16 22:05:25
 * @LastEditors: lemonlqf lemonlqf@outlook.com
 * @LastEditTime: 2026-09-17 20:12:31
 * @FilePath: \picmap-go\frontend\src\i18n\lang\zh-CN.ts
 * @Description: 
 */
export default {
  undefined: '未知',
  defaultAvatar: '默认头像',
  positioningGroup: '定位分组',
  confirm: '确认',
  cancel: '取消',
  userManagement: '用户管理',
  userNumber: '用户数量',
  createTime: '创建时间',
  userInfo: '用户信息',
  pictureInfo: '图片信息',
  mediaInfo: '媒体信息',
  name: '名称',
  userName: '用户名称',
  groupName: '分组名称',
  tileName: '瓦片名称',
  language: '语言',
  pictureNumber: '图片数量',
  videoNumber: '视频数量',
  trackNumber: '轨迹数量',
  groupNumber: '分组数量',
  repositioning: '重新定位',
  dissolveGroup: '解散分组',
  deleteGroup: '删除分组',
  deletePicture: '删除图片',
  deleteVideo: '删除视频',
  deleteUser: '删除用户',
  delete: '删除',
  deleteSuccess: '删除成功',
  distance: '距离',
  startTime: '开始时间',
  endTime: '结束时间',
  actions: '操作',
  setGroup: '设置分组',
  setPanorama: '设为全景',
  cancelPanorama: '取消全景',
  littlePlanet: '小星球',
  normalPanorama: '普通全景',
  fullscreen: '全屏',
  exitFullscreen: '退出全屏',
  fixPosition: '确定位置',
  createUser: '创建新用户',
  pictureDescription: '图片描述',
  exitGroup: '退出分组',
  year: '年',
  month: '月',
  day: '日',
  downloadPicture: '下载原图',
  pictureLoadFailed: '图片加载失败',
  timeInfo: '时间信息',
  GPSInfo: 'GPS信息',
  cameraInfo: '相机信息',
  noData: '无数据',
  title: '标题',
  latitude: '纬度',
  longitude: '经度',
  altitude: '海拔',
  dateTime: '拍摄时间',
  arits: '图像作者',
  softWare: '图像软件',
  make: '相机制造商',
  model: '相机型号',
  fNumber: '光圈值',
  exposureTime: '曝光时间',
  ISOSpeedRatings: 'ISO速度',
  exposureBiasValue: '曝光补偿',
  focalLength: '焦距',
  maxApertureValue: '最大光圈',
  resolution: '分辨率',
  brightnessValue: '亮度',
  size: '图片大小',
  defaultTitle: '默认标题',
  unlocated: '未定位',
  createGroup: '创建新分组',
  setGroupInfo: '设置分组信息',
  targetGroup: '目标分组',
  joinNewGroup: '加入新分组',
  newGroupName: '新分组名称',
  newGroupLocation: '新分组位置',
  newGroupLongitude: '新分组经度',
  newGroupLatitude: '新分组纬度',
  newGroupAltitude: '新分组海拔',
  manualLocate: '手动定位',
  autoLocate: '自动定位',
  groupInfo: '分组信息',
  addGroup: '添加分组',
  uploadedPicture: '已上传图片',
  uploadPicture: '上传图片',
  uploadVideo: '上传视频',
  uploadedVideo: '已上传视频',
  play: '播放',
  uploadTrack: '上传轨迹',
  trackManagement: '轨迹管理',
  gpxSizeLimit: 'GPX文件上传限制:50MB',
  trackColor: '轨迹颜色',
  selectFile: '选择文件',
  fold: '折叠',
  expand: '展开',
  save: '保存',
  yes: '是',
  no: '否',
  pictureToBeUploaded: '待上传图片',
  locate: '定位',
  upload: '上传',
  group: '分组',
  videoLabel: '视频',
  trackLabel: '轨迹',
  batchUpload: '批量上传',
  batchUploadToGroup: '批量上传到分组',
  batchDeleteImages: '批量删除',
  batchAddToGroup: '批量添加到分组',
  selectedCount: '已选',
  nodes: '个节点',
  uploadingPleaseWait: '正在上传中，请稍等...',
  clear: '清空',
  clearAll: '全部清空',
  mediaToBeUploaded: '待上传图片/视频',
  located: '已定位',
  linked: '已关联',
  linkTrack: '关联轨迹',
  settingSuccess: '设置成功！',
  pictureName: '照片名',
  chooseAppropriateImageArea: '请选择合适的图片区域',
  rotateLeft: '向左旋转',
  rotateRight: '向右旋转',
  zoomIn: '放大',
  zoomOut: '缩小',
  clickToUpliadCover: '点击上传封面',
  NonEditableDefaultTile: '不可编辑默认瓦片',
  GSatellite: '高德卫星图',
  GVector: '高德矢量图',
  GVector3D: '高德3D图',
  showOverlay: '展示路网',
  tileOverlays: '叠加层',
  addOverlay: '添加叠加层',
  overlayName: '名称',
  overlayUrl: '地址',
  noOverlay: '暂无叠加层',
  notSet: '未设置',
  edit: '编辑',
  exitEdit: '退出编辑',
  address: '地址',
  changeUser: '切换用户',
  setting: '设置',
  picture: '图片',
  pureMode: '纯净模式',
  zoomUpMap: '放大地图',
  zoomDownMap: '缩小地图',
  initializationCenter: '初始化中心',
  infoEdit: '信息编辑',
  backToPicMap: '返回相册地图',
  mapManagement: '地图管理',
  loading: '加载中',
  tileSetting: '瓦片设置',
  map: '地图',
  user: '用户',
  setPictureLocation: '设置图片位置',
  dataManagement: '数据',
  backup: '备份',
  createBackup: '创建备份',
  backupName: '备份名称',
  backupNamePlaceholder: '请输入备份名称',
  backupNameExists: '备份名称已存在',
  backupTip: '备份将包含所有用户数据、图片和设置',
  backupHistory: '备份历史',
  noBackup: '暂无备份',
  restore: '恢复',
  restoreData: '恢复数据',
  restoreFromFile: '从文件恢复',
  backupFile: '备份文件',
  restoreWarning: '恢复将覆盖当前数据（覆盖模式下），请确认已提前创建备份',
  coverMode: '覆盖模式',
  mergeMode: '合并模式',
  coverModeTip: '用备份数据完全替换现有数据，现有数据将丢失',
  mergeModeTip: '将备份数据与现有数据合并，保留所有现有数据',
  backupSuccess: '备份成功',
  backupFailed: '备份失败',
  backupCancelled: '已取消备份',
  backupLargeWarning: '存档较大，备份时间可能较长，请耐心等待',
  backupLoading: '正在备份，请稍候...',
  restoreSuccess: '恢复成功，正在重新加载...',
  restoreFailed: '恢复失败',
  confirmDeleteBackup: '确定要删除此备份吗？',
  warning: '警告',
  deleteFailed: '删除失败',
  reset: '重置',
  timeline: '时间轴',

  // 时区
  timezone: {
    saveSuccess: '时区已保存，重新打开相关弹框后生效'
  },

  // 轨迹相关
  track: {
    basicInfo: '基础信息',
    speedData: '速度数据',
    elevationData: '海拔数据',
    healthData: '健康数据',
    movingTime: '移动时间',
    totalTime: '总时间',
    movingSpeed: '移动速度',
    totalSpeed: '总速度',
    maxSpeed: '最大速度',
    movingPace: '移动配速',
    elevationMax: '最高海拔',
    elevationMin: '最低海拔',
    elevationGain: '累计爬升',
    elevationLoss: '累计下降',
    averageHr: '平均心率',
    averageCadence: '平均踏频',
    averageTemp: '平均温度',
    unnamed: '未命名轨迹',
    showOnMainMap: '显示在主地图',
    removeFromMap: '从地图移除',
    boundVideos: '关联视频',
    videoPlay: '视频播放',
    trackMap: '轨迹地图',
    prevVideo: '上一个视频',
    nextVideo: '下一个视频',
    autoNext: '自动播放'
  },

  icon: {
    iconLibrary: '图标库',
    avatarIcons: '头像图标',
    trackIcons: '轨迹图标',
    uploadIcon: '上传图标',
    uploadSuccess: '图标上传成功',
    uploadFailed: '图标上传失败',
    startIcon: '起点图标',
    endIcon: '终点图标'
  },

  storage: {
    storageDir: '存储目录',
    archiveDir: '数据目录',
    backupDir: '备份目录',
    selectDir: '选择',
    save: '保存',
    changeTip: '修改数据目录后，新目录只有初始化数据。请在修改前先创建备份（含图标等所有数据），修改并重启后，再在备份历史中导入恢复，否则原有数据将无法在新目录看到。',
    changeConfirm: '修改数据目录后需重启应用生效，且新目录只有初始化数据。请务必先在“备份”中创建备份，重启后导入恢复，否则原数据将丢失。确定继续吗？',
    saveSuccess: '存储目录保存成功，重启后生效',
    saveFailed: '存储目录保存失败',
    selectBackupFirst: '请先选择备份文件',
    coverWarning: '恢复备份会覆盖当前数据（覆盖模式下），请确认已提前创建备份。确定继续吗？'
  },

  description: {
    ensureToDissolveGroup: '确定要解散分组吗？该分组将会被解散，组内图片/视频仍然保留！',
    ensureToDeleteGroup: '确定要删除分组吗？该分组将会被删除，组内图片不会保留，组内视频仅解除关联！',
    groupNotExist: '分组还未添加到地图中！',
    enterUserName: '"请输入用户名！',
    userNameExist: '用户名已存在！',
    groupNameExist: '分组名称已存在！',
    enterGroupName: '请输入分组名称！',
    enterLongitude: '请输入经度！',
    enterNumber: '请输入数字！',
    editGroupInfoSuccess: '分组信息设置成功!',
    editGroupInfoFail: '分组信息设置失败!',
    activeOneTile: '请最少启用一个瓦片！',
    dissolveSuccess: '解散成功！',
    selectOrCreateGroup: '请选择分组或者新建分组！',
    analysisPicture: '图片数据解析中',
    needGPSInfo: '图片无地址信息，无法直接上传！',
    noPictureCanUpload: '无图片可上传，请完善图片定位信息或上传新图片！',
    pictureUploadedSuccess: '图片上传成功！',
    somePictureUploadedSuccess: '部分图片上传成功！',
    needLongitude: '经度值不能为空！',
    needLatitude: '纬度值不能为空！',
    enterName: '请输入名称',
    needTileUrl: '地址为空时，瓦片不会显示',
    needName: '姓名不能为空',
    nameExist: '姓名已存在',
    canNotCreateUserWhenEdit: '请先退出编辑再新建用户',
    confirmDeleteUser: '确定要删除用户吗？该用户的所有的图片和视频也会被删除，不可恢复！',
    setUrlToPreview: '请添加有效瓦片地址,在此处预览',
    setAsDefault: '设为默认瓦片',
    largeSize: '图片尺寸过大, 超过 ',
    uploadFailed: '上传失败!',
    selectFileFirst: '请先选择文件！',
    trackUploadedSuccess: '轨迹上传成功！',
    parsePictureInfoFailed: '解析图片信息失败!',
    parsePictureFailed: '解析图片失败!',
    loadTrackFailed: '加载轨迹失败!',
    deleteSuccess: '删除成功!',
    deleteFailed: '删除失败!',
    updateSuccess: '更新成功!',
    updateFailed: '更新失败!',
    trackExistConfirm: '已存在同名文件，是否覆盖上传？',
    trackNameDuplicate: '轨迹名称已存在，请使用其他名称',
    closeUploadTrackConfirm: '还有未上传的轨迹数据，关闭将会丢失这些数据，确定要关闭吗？',
    warning: '警告',
    searchPlaceholder: '搜索名称或时间',
    selectTrackToLinkVideo: '请选择一条轨迹，点击"对齐视频"将该视频关联到轨迹并自动上传',
    noMediaCanUpload: '无图片/视频可上传，请完善定位信息或上传新文件！',
    mediaUploadedSuccess: '上传成功！',
    someMediaUploadedSuccess: '部分上传成功！',
    needVideoGPSInfo: '视频无定位信息，请先手动定位或关联轨迹后再导入！',
    selectVideoFailed: '选择视频失败！',
    videoImportFailed: '视频导入失败！',
    videoImportSuccess: '视频导入成功！',
    videoImportFailedNamed: '视频「{name}」导入失败',
    videoNoLocation: '该视频暂无定位信息',
    videoLinkedTrackNoLocation: '该视频关联轨迹，无独立定位坐标',
    videoNeedGpsOrLocate: '视频需要内嵌 GPS 或手动定位后才能导入',
    videoLoadFailed: '视频加载失败',
    trackNotExist: '轨迹不存在',
    videoLinkedAndUploaded: '视频已关联轨迹并上传',
    alignSaved: '对齐已保存',
    nodeExist: '节点已存在！请编辑已有节点',
    noMarkerInstance: '没有传入节点实例',
    noImageId: '没有传入图片id'
  },
  placeholder: {
    selectGroup: '请选择分组'
  }
}